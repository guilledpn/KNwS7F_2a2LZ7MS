-- Issue #24 · permitir corregir observaciones ya completadas.
-- La muestra puede estar active o completed; regrabar un ítem conserva completed_at
-- y recalcula el estado de la cabecera según queden pendientes o no.
...
begin
  ...
  update public.crm_analysis_samples
     set status = case when v_pending = 0 then 'completed' else 'active' end,
         updated_at = now()
   where sample_id = v_sample_id
