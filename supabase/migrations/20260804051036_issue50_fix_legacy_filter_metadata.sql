-- Issue #50 · completar opciones, alinear conteos y mostrar campaña completa.
begin;
...
declare
  ...
  v_rows jsonb := '[]'::jsonb;
  v_total bigint := 0;
  v_result_pending bigint := 0;
  v_result_assigned bigint := 0;
  v_base bigint := 0;
  v_pending bigint := 0;
  v_assigned bigint := 0;
begin
  ...
  month_selected as materialized (
    select
      mm.contact_id,
      count(distinct mm.period)::integer as matched_periods
    from public.contact_month_state mm
    where v_month_count > 0
      and mm.visible
      and mm.period = any(v_months)
    group by mm.contact_id
  ),
  candidates as materialized (
    select b.*
    from base b
    left join month_selected ms on ms.contact_id = b.contact_id
    where
      (
        v_search = ''
      ...
      and (
        v_month_count = 0
        or (v_month_mode = 'any' and coalesce(ms.matched_periods,0) > 0)
        or (
          v_month_mode in ('all','only')
          and coalesce(ms.matched_periods,0) = v_month_count
        )
      )
  ),
  filtered as materialized (
    ...
    where
      v_month_count = 0
      or v_month_mode <> 'only'
      or not exists(
        select 1
        from public.contact_month_state outside_month
        where outside_month.contact_id = c.contact_id
          and outside_month.visible
          and not (outside_month.period = any(v_months))
      )
  ),
  page as (
    select f.*
  ...
  select
    ...
    coalesce((select count(*) from filtered where estado_key = 'pendiente'),0),
    coalesce((select count(*) from filtered where origen = 'asignado'),0),
    coalesce((
  ...
  into v_total,v_result_pending,v_result_assigned,v_rows;

  return jsonb_build_object(
    'ok',true,
    'source','get_contacts_v2_filtered_issue50',
    'active_period',v_period,
    'rows',v_rows,
    'result_total',v_total,
    'result_pending',v_result_pending,
    'result_assigned',v_result_assigned,
    'base_total',v_base,
...
as $function$
  ...
  queue_options as materialized (
    select distinct cp.campaign_name,cp.campaign_desc
    from public.work_queue w
    cross join params p
    join public.campaigns cp on cp.campaign_id = w.campaign_id
    where w.period = p.period
      and w.visible
  )
  select jsonb_build_object(
    'ok',true,
    'source','get_contacts_v2_filter_options_issue50',
    'months',coalesce((
      select jsonb_agg(x.period order by x.period desc)
      from (
        select distinct cp.period
        from public.campaigns cp
        where nullif(trim(cp.period),'') is not null
      ) x
    ),'[]'::jsonb),
    'campaigns',coalesce((
      select jsonb_agg(x.campaign_name order by x.campaign_name)
      from (
        select distinct qo.campaign_name
        from queue_options qo
        where nullif(trim(qo.campaign_name),'') is not null
      ) x
    ),'[]'::jsonb),
    'descriptions',coalesce((
      select jsonb_agg(x.campaign_desc order by x.campaign_desc)
      from (
        select distinct qo.campaign_desc
        from queue_options qo
        where nullif(trim(qo.campaign_desc),'') is not null
      ) x
    ),'[]'::jsonb)
  );
$function$;
...
comment on function public.get_contacts_v2_filtered(
  ...
  'Issue #50: conteos del mismo resultado filtrado; no modifica work_queue.';
comment on function public.get_contacts_v2_filter_options(text) is
  'Issue #50: opciones completas y estables de la cola visible sin muestreos parciales.';
