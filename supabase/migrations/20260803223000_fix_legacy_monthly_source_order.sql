-- Preserve the canonical row order of each monthly TOTAL source and use it
-- to order rule-managed contacts without changing eligibility or UI behavior.
...
create index if not exists monthly_source_order_period_row_idx
  on public.monthly_source_order(period, source_row, contact_id);
...
comment on table public.monthly_source_order is
  'Canonical global row position of each contact in a monthly TOTAL source. Read through SECURITY DEFINER functions; not exposed to Legacy clients.';
comment on column public.monthly_source_order.source_row is
  'One-based data-row position in the complete monthly source, excluding the header.';
...
select
  ...
  case
    ...
    when lvs.period ~ '^[0-9]{4}-[0-9]{2}$'
     and p_period ~ '^[0-9]{4}-[0-9]{2}$' then
      least(
        1999999999,
        1000000
        + greatest(
            0,
            ((substring(p_period,1,4)::integer * 12 + substring(p_period,6,2)::integer)
             - (substring(lvs.period,1,4)::integer * 12 + substring(lvs.period,6,2)::integer))
          ) * 1000000
        + least(coalesce(mso.source_row,lvs.import_order,999999),999999)
      )::integer
    when lvs.period is not null then
      least(1999999999,1000000000 + least(coalesce(mso.source_row,lvs.import_order,999999),999999))::integer
    else 2000000000
...
left join public.monthly_source_order mso
  on mso.contact_id = c.contact_id
 and mso.period = lvs.period
$function$;

comment on function public.contact_eligibility_for_period(text) is
  'Canonical monthly eligibility. Assigned contacts keep assigned order; rule contacts are ordered by newest source month and global source row.';
...
declare
  ...
  v_source_order_rows integer := 0;
begin
  perform set_config('statement_timeout','120000',true);

  -- The Legacy importer keeps the staging session until the queue rebuild.
  -- Capture the exact complete-file order here, before staging cleanup.
  select s.import_session_id
    into v_source_session
  from public.staging_contacts s
  where s.period = p_period
    and lower(trim(coalesce(s.load_type,''))) = 'mensual'
  group by s.import_session_id
  order by max(s.created_at) desc, s.import_session_id desc
  limit 1;

  if v_source_session is not null then
    with src as (
      select distinct on (s.rut_norm)
        s.period,
        s.rut_norm,
        coalesce(s.import_order,s.staging_id)::integer as source_row
      from public.staging_contacts s
      where s.import_session_id = v_source_session
        and s.period = p_period
        and lower(trim(coalesce(s.load_type,''))) = 'mensual'
      order by
        s.rut_norm,
        coalesce(s.import_order,s.staging_id) asc,
        s.staging_id asc
    ), upserted_order as (
  ...
  return jsonb_build_object(
    'ok', true,
    'period', p_period,
    'rule', 'canonical_monthly_eligibility_v1_source_order',
    ...
    'source_order_session', v_source_session,
    'source_order_rows', v_source_order_rows
  );
end
$function$;

comment on function public.rebuild_work_queue_for_period(text) is
  'Rebuilds the queue and captures canonical monthly source order from the active Legacy staging session before cleanup.';
