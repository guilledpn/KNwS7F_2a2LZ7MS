-- Parte 3/6 · validación del estado aplicado.
...
begin
  ...
  create unique index issue63_expected_queue_contact_idx
    on pg_temp.issue63_expected_queue(contact_id);

  select count(*)::integer into v_expected_missing
  from pg_temp.issue63_expected_queue expected
  left join public.work_queue w
    on w.contact_id=expected.contact_id and w.period=v_operation.period and w.visible
  where w.contact_id is null;

  select count(*)::integer into v_unexpected_visible
  from public.work_queue w
  left join pg_temp.issue63_expected_queue expected on expected.contact_id=w.contact_id
  where w.period=v_operation.period and w.visible and expected.contact_id is null;
