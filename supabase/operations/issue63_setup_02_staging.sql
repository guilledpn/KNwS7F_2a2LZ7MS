-- Parte 2/4 · staging, validación de archivos y estado agregado.
...
begin
  ...
  select count(*)::integer into v_chunk_rows
  ...
  if p_start_order < 1 or v_end_order > v_manifest.expected_rows then
    raise exception 'Chunk range is outside the configured file' using errcode='22023';
  end if;
  ...
  if exists (
    ...
    raise exception 'Every payload row must contain exactly 11 columns' using errcode='22023';
  end if;
  ...
  if exists (
    ...
    where fields[1] !~ '^[0-9]+[0-9K]$'
       or fields[11] not in ('Gestionado','No Gestionado')
       or fields[8] = '' or fields[9] = '' or fields[10] = ''
  ) then
  ...
  if exists (
    with parsed as (
      select p_start_order + ordinality::integer - 1 as source_order,line
  ...
  select
    v_operation.operation_key,p_file_name,p_load_type,
    p_start_order + p.rn - 1,p.line,
...
begin
  ...
  loop
    ...
    where operation_key=v_operation.operation_key and file_name=v_manifest.file_name;
    ...
    from (
      ...
      from issue63_ops.stage_rows
      where operation_key=v_operation.operation_key and file_name=v_manifest.file_name
      group by campaign_key order by campaign_key
    ) x;
    ...
    if v_rows <> v_manifest.expected_rows
      ...
      raise exception 'Staged file % does not match its manifest',v_manifest.file_name
        using errcode='55000';
    end if;
  end loop;
  ...
  left join issue63_ops.stage_rows t
    on t.operation_key=a.operation_key and t.load_type='mensual'
   and t.rut_norm=a.rut_norm and t.campaign_key=a.campaign_key
  where a.operation_key=v_operation.operation_key
    and a.load_type='asignado' and t.rut_norm is null;
  ...
  join issue63_ops.stage_rows t
    on t.operation_key=a.operation_key and t.load_type='mensual'
   and t.rut_norm=a.rut_norm and t.campaign_key=a.campaign_key
  where a.operation_key=v_operation.operation_key and a.load_type='asignado'
  ...
  if v_missing_assigned <> 0 or v_field_mismatches <> 0 then
    raise exception 'ASIGNADOS is not an exact subset of TOTAL' using errcode='55000';
  end if;
