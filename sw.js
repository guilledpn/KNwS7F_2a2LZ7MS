const APP_VERSION='crm-llamados-github-wrapper-v69';
...
const V2_TOTAL=25840;
const V2_PENDING=25755;
const V2_ASSIGNED=234;
...
function h(req){const o={};['apikey','authorization','x-client-info'].forEach(k=>{const v=req.headers.get(k);if(v)o[k]=v;});o.accept='application/json';return o;}
...
async function fallbackGetContactsV2(req){
  ...
  const search=String(p.p_search||'').trim();
  const months=Array.isArray(p.p_months)?p.p_months:[];
  const types=Array.isArray(p.p_types)?p.p_types:[];
  const monthMode=p.p_month_mode||'any';
  if(search||months.length||monthMode!=='any')return fetch(req);
  const headers=h(req);
  const cond=[];
  ...
  if(types.length===1&&types[0]==='regla')cond.push('motivo_gestionabilidad=eq.disponible_por_regla');
  if(types.length>1)cond.push('motivo_gestionabilidad=in.(asignado,disponible_por_regla)');
  ...
  const q='/rest/v1/contact_operational_state_v2?select='+select+'&'+cond.join('&')+'&'+order+'&limit='+limit+'&offset='+offset;
  const baseFetch=fetch(SUPA+q,{headers,mode:'cors'});
  let totalPromise;
  if(situation==='gestionables'&&!types.length&&!p.p_pending_only&&!p.p_assigned_only)totalPromise=Promise.resolve(V2_TOTAL);else totalPromise=exactCount('/rest/v1/contact_operational_state_v2?select=rut_norm&'+cond.join('&')+'&limit=1',headers);
  const [baseRows,total]=await Promise.all([baseFetch.then(r=>{if(!r.ok)throw new Error('fallback base '+r.status);return r.json();}),totalPromise]);
  const ruts=[...new Set(baseRows.map(x=>x.rut_norm).filter(Boolean))];
  let contactMap={};
  if(ruts.length){
    const cq='/rest/v1/contacts?select=contact_id,rut_norm,rut,nombre,telefono_1,telefono_2,telefono_3,email,telefono_activo_idx&rut_norm=in.'+encodeURIComponent(inList(ruts));
  ...
  const isDefault=situation==='gestionables'&&!types.length&&!p.p_pending_only&&!p.p_assigned_only;
  return okJson({ok:true,active_period:ACTIVE_PERIOD,limit,offset,base_total:Number(total||0),base_pending:isDefault?V2_PENDING:Number(total||0),base_assigned:isDefault?V2_ASSIGNED:0,base_gestionables:isDefault?V2_TOTAL:Number(total||0),base_no_gestionables:0,result_total:Number(total||0),rows});
}
self.addEventListener('fetch',event=>{const req=event.request;if(req.method==='POST'&&req.url.startsWith(SUPA+'/rest/v1/rpc/get_contacts_v2')){event.respondWith(fallbackGetContactsV2(req).catch(()=>fetch(req)));return;}if(req.method!=='GET')return;event.respondWith(fetch(req).catch(()=>caches.match(req)));});
