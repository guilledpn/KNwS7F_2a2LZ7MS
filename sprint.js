(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      #crm-sprint-chip{height:38px;border:0;border-radius:19px;background:#0f172a;color:#fff;font-weight:800;font-size:12.5px;padding:0 11px;display:flex;align-items:center;gap:6px;white-space:nowrap;box-shadow:0 6px 16px rgba(15,23,42,.16)}
      #crm-sprint-chip.work{background:#166534}#crm-sprint-chip.break{background:#92400e}
      ...
      #crm-sprint-stats-card{background:#fff;border:1px solid rgba(198,208,224,.9);border-radius:24px;margin:0 0 14px;padding:16px;box-shadow:0 1px 3px rgba(15,23,42,.06),0 8px 22px rgba(15,23,42,.04)}#crm-sprint-stats-card h3{margin:0 0 10px;font-size:18px}.crm-sprint-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.crm-sprint-metric{background:#f6f9ff;border:1px solid #dbe5f3;border-radius:18px;padding:12px}.crm-sprint-metric b{display:block;font-size:22px;color:#0f172a}.crm-sprint-metric span{font-size:12px;color:#64748b}.crm-sprint-log{margin-top:12px;color:#475569;font-size:13px;line-height:1.35}
      @media(max-width:430px){#crm-sprint-chip{font-size:11.5px;padding:0 9px}.goal-chip{min-width:auto!important;padding:0 9px!important}.topbar{gap:8px!important}}
    `;
  ...
  function startSprint() {
    const state = load();
    const now = Date.now();
    state.active = { mode: 'work', startedAt: now, endAt: now + Math.max(1, Number(state.workMin || 20)) * 60000 };
    save(state);
  ...
  function finishPhase(state) {
    ...
    if (state.active.mode === 'work') {
      ...
      state.active = { mode: 'break', startedAt: Date.now(), endAt: Date.now() + Math.max(1, Number(state.breakMin || 5)) * 60000 };
      save(state);
  ...
  function update() {
    addStyles();
    const state = load();
    if (state.active && Date.now() >= state.active.endAt) {
      finishPhase(state);
      return;
    }
    ...
    if (time) time.textContent = state.active ? fmt(state.active.endAt - Date.now()) : fmt((state.workMin || 20) * 60000);
    if (sub) sub.textContent = state.active ? (state.active.mode === 'work' ? 'Sprint en curso' : 'Descanso en curso') : (state.workMin + ' minutos de llamados + ' + state.breakMin + ' de descanso');
